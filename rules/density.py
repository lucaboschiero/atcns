...
def fun(input) :                # input contains the clients weight updates
    ...
    for k in range(n) :                   # loop to exclude attacker nodes, one at a time, and recumputing the density after each elimination
        ...
        logger.info(f"Node:  {i}, Original density: {o}, Updated density: {d}")
    ...
    if len(h) < 2:      # if the number of attackers is too small (<2), all nodes are considered as attackers and excluded in the weight update calculation
        ...
        logger.info(f"Attackers: {[i for i in range(n) if i not in h]}")         # attacker nodes are the ones that are not in h
        ...
        logger.info(f"Attackers:  {h}")       # attackers
        print(avg)
        print(den)
        return out,h
        ...
        logger.info(f"Attackers: {[i for i in range(n) if i not in h]}")        # attackers (not in h)
        print(den)
        print(avg)
        return out,[i for i in range(n) if i not in h]
